...
---
### Prerequisite
[Setup cloudnative-pg database](../../posts/setup-cloudnativ-pg-database)
### Goal
...
-  So, let's add the `sonarr` secret in Bitwarden Secrets Manager. Create the new secret named `sonarr` and add the key:values pairs:
...
However, as you continue adding new apps, you will notice quickly that this dependency tree will change. Some apps don't require a database, while some depend on multiple other apps to function, each with their own requirements. Some apps require connectivity to additional servers, local hardware, or cloud services which each require differing resources based on the scale of your project. How much time and money you invest into your lab should realistically reflect what you hope to get out of your project. Hopefully, you can now use your cluster to play around with different applications and decide what level of commitment is right for your use case.

### Next Steps
If you're feeling brave, now would be a good time to try writing your own standalone app which can be also be deployed via flux. You can build an app and then push the container image or helm chart to github, or you can host your own registry in your cluster via [Harbor](https://goharbor.io/), a free open-source registry.

#### Helm Chart Releaser
- To be posted!
#### Harbor
I have been able to deploy Harbor and host images there in the past, but I ran into issues pulling container images for some applications. Check `kubesearch.dev` for example deployments from others. 
